...
from p22_packets import blinded_packets
...
def stable_model_offset(model_key: str) -> int:
    digest = hashlib.sha256(("P22:" + model_key).encode("utf-8")).hexdigest()
...
def main() -> None:
    ...
    out.write_text(
        json.dumps(run(args.model), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
